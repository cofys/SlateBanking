...
- `/api/auth/citycorp/callback` and `/api/auth/callback` handle CityCorp OAuth authorization code exchange (`POST https://api.cityrp.org/auth/token`).
  - **State-Encoded Parameter Parity**: Embeds the exact `appId` and `redirectUri` used during the `/authorize` link generation directly into the OAuth `state` payload. Upon callback at `/api/auth/citycorp/callback` or `/api/auth/callback`, the exact `app_id` and `redirect_uri` are decoded and submitted alongside the `client_secret` authorization code to `https://api.cityrp.org/auth/token` with the bank's `cityCorpAppSecret`, guaranteeing 100% parameter symmetry and eliminating `invalid_grant` errors.
